...
import com.duylongtech.backend.feature.assembly.AssemblyOrderLineRequest;
import com.duylongtech.backend.feature.assembly.AssemblyOrderLineResponse;
import com.duylongtech.backend.feature.assembly.AssemblyOrderMapper;
import com.duylongtech.backend.feature.assembly.AssemblyOrderRepository;
import com.duylongtech.backend.feature.assembly.AssemblyOrderRequest;
import com.duylongtech.backend.feature.assembly.AssemblyOrderResponse;
...
public class AssemblyOrderService {
    ...
    private final com.duylongtech.backend.feature.assembly.AssemblyOrderMapper assemblyOrderMapper;
    ...
    @Transactional
    public void generateInventoryDocument(Long id, com.duylongtech.backend.feature.inventory.GenerateInventoryDocumentRequest request, String actor) {
        AssemblyOrder order = findOrderOrThrow(id);
        ...
        com.duylongtech.backend.feature.inventory.InventoryDocumentRequest docReq = new com.duylongtech.backend.feature.inventory.InventoryDocumentRequest();
        ...
        List<com.duylongtech.backend.feature.inventory.InventoryDocumentLineRequest> lines = request.getLines().stream().map(line -> {
            com.duylongtech.backend.feature.inventory.InventoryDocumentLineRequest lr = new com.duylongtech.backend.feature.inventory.InventoryDocumentLineRequest();
    ...
    private AssemblyOrderResponse toOrderResponse(AssemblyOrder order) {
        ...
        if (order.getCreatedBy() != null) {
            response.setCreatedByName(userRepository.findById(order.getCreatedBy())
                    .map(com.duylongtech.backend.feature.auth.User::getFullName)
                    .orElse(String.valueOf(order.getCreatedBy())));
        }
        
        List<com.duylongtech.backend.feature.assembly.AssemblyOrderSerial> serials = assemblyOrderSerialRepository.findByAssemblyOrderId(order.getId());
        List<com.duylongtech.backend.feature.assembly.AssemblyOrderSerialResponse> mappedSerials = serials.stream()
                .filter(this::isOrderSerialMapping)
    ...
    @Transactional(readOnly = true)
    public List<com.duylongtech.backend.feature.inventory.InventoryDocumentResponse> getOrderDocuments(Long id) {
        findOrderOrThrow(id);
    ...
    private void createDocumentPair(AssemblyOrder order) {
        com.duylongtech.backend.feature.inventory.InventoryDocumentRequest export = baseDocument(order);
        com.duylongtech.backend.feature.inventory.InventoryDocumentRequest receipt = baseDocument(order);
        List<com.duylongtech.backend.feature.inventory.InventoryDocumentLineRequest> componentLines = order.getLines().stream()
                .map(line -> documentLine(line.getComponentVariant().getId(), line.getQuantityRequired()))
                .toList();
        List<com.duylongtech.backend.feature.inventory.InventoryDocumentLineRequest> targetLines =
    ...
    private com.duylongtech.backend.feature.inventory.InventoryDocumentRequest baseDocument(AssemblyOrder order) {
        com.duylongtech.backend.feature.inventory.InventoryDocumentRequest request =
                new com.duylongtech.backend.feature.inventory.InventoryDocumentRequest();
    ...
    private com.duylongtech.backend.feature.inventory.InventoryDocumentLineRequest documentLine(Long variantId, BigDecimal quantity) {
        com.duylongtech.backend.feature.inventory.InventoryDocumentLineRequest line =
                new com.duylongtech.backend.feature.inventory.InventoryDocumentLineRequest();
    ...
    private List<com.duylongtech.backend.feature.inventory.InventoryDocumentLineRequest> asExportLines(
            List<com.duylongtech.backend.feature.inventory.InventoryDocumentLineRequest> lines) {
        lines.forEach(line -> line.setQuantityOut(line.getBaseQuantity()));
        return lines;
    }

    private List<com.duylongtech.backend.feature.inventory.InventoryDocumentLineRequest> asImportLines(
            List<com.duylongtech.backend.feature.inventory.InventoryDocumentLineRequest> lines) {
