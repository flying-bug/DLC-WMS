...
public class InventoryDocumentController {
    ...
    @PreAuthorize("hasAuthority('export:view') or hasAuthority('assembly:view') or hasAuthority('repair:view')")
    ...
    public ApiResponse<java.util.List<InventoryDocumentResponse>> createExportFromSalesOrder(
        ...
        Long actorUserId = userRepository.findByUsername(actor)
                .map(com.duylongtech.backend.feature.auth.User::getId).orElse(1L);
    ...
    public ApiResponse<com.duylongtech.backend.feature.inventory.DependencyCheckResponse> checkUnpost(@PathVariable Long id) {
    ...
    public ApiResponse<List<com.duylongtech.backend.feature.audit.AuditLogResponse>> getExportLogs(@PathVariable Long id) {
        org.springframework.data.domain.Page<com.duylongtech.backend.feature.audit.AuditLog> page = auditLogService.getLogsForEntity("ExportSlip", id, 0, 100);
        List<com.duylongtech.backend.feature.audit.AuditLogResponse> logs = page.getContent().stream()
                .map(l -> com.duylongtech.backend.feature.audit.AuditLogResponse.builder()
                        .id(l.getId())
