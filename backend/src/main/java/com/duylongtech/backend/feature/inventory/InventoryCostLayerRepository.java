...
public interface InventoryCostLayerRepository extends JpaRepository<InventoryCostLayer, Long> {
    ...
    @Query("SELECT c FROM InventoryCostLayer c WHERE c.variantId = :variantId AND c.quantityLayered > 0 ORDER BY c.createdAt ASC")
    List<InventoryCostLayer> findAvailableLayersByVariant(@Param("variantId") Long variantId);

    @Query("SELECT c FROM InventoryCostLayer c WHERE c.variantId IN :variantIds AND c.quantityLayered > 0 ORDER BY c.createdAt ASC")
