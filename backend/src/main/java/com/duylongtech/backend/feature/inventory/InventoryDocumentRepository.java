...
public interface InventoryDocumentRepository extends JpaRepository<InventoryDocument, Long> {
    ...
    @Query("SELECT DISTINCT e FROM InventoryDocument e LEFT JOIN FETCH e.lines l WHERE e.docType = 'EX_SO' AND (:keyword IS NULL OR (LOWER(e.docCode) LIKE LOWER(CONCAT('%', TRIM(:keyword), '%')) OR EXISTS (SELECT 1 FROM InventoryDocumentLine line LEFT JOIN ProductVariant pv ON line.variantId = pv.id WHERE line.inventoryDocument = e AND (LOWER(line.serialNumbersText) LIKE LOWER(CONCAT('%', TRIM(:keyword), '%')) OR LOWER(pv.barcode) LIKE LOWER(CONCAT('%', TRIM(:keyword), '%')) OR LOWER(pv.sku) LIKE LOWER(CONCAT('%', TRIM(:keyword), '%')) OR LOWER(pv.variantName) LIKE LOWER(CONCAT('%', TRIM(:keyword), '%')))))) AND (:warehouseId IS NULL OR e.warehouseId = :warehouseId) AND (:allowedWarehouseIds IS NULL OR e.warehouseId IN :allowedWarehouseIds) AND (:status IS NULL OR e.status = :status) AND (:issuePurpose IS NULL OR e.issuePurpose = :issuePurpose) AND (:referenceType IS NULL OR e.referenceType = :referenceType OR (:referenceType = 'MULTI_TECH' AND e.referenceType IN ('REPAIR', 'ASSEMBLY_ORDER'))) AND (:referenceId IS NULL OR e.referenceId = :referenceId) AND (:partnerId IS NULL OR e.partnerId = :partnerId) AND (:salespersonId IS NULL OR e.salespersonId = :salespersonId) AND (:fromDate IS NULL OR e.docDate >= :fromDate) AND (:toDate IS NULL OR e.docDate <= :toDate) ORDER BY e.updatedAt DESC, e.id DESC")
    ...
    @Query("SELECT DISTINCT e FROM InventoryDocument e LEFT JOIN FETCH e.lines l WHERE e.docType = 'IN_PO' AND (:keyword IS NULL OR (LOWER(e.docCode) LIKE LOWER(CONCAT('%', TRIM(:keyword), '%')) OR EXISTS (SELECT 1 FROM InventoryDocumentLine line LEFT JOIN ProductVariant pv ON line.variantId = pv.id WHERE line.inventoryDocument = e AND (LOWER(line.serialNumbersText) LIKE LOWER(CONCAT('%', TRIM(:keyword), '%')) OR LOWER(pv.barcode) LIKE LOWER(CONCAT('%', TRIM(:keyword), '%')) OR LOWER(pv.sku) LIKE LOWER(CONCAT('%', TRIM(:keyword), '%')) OR LOWER(pv.variantName) LIKE LOWER(CONCAT('%', TRIM(:keyword), '%')))))) AND (:warehouseId IS NULL OR e.warehouseId = :warehouseId) AND (:allowedWarehouseIds IS NULL OR e.warehouseId IN :allowedWarehouseIds) AND (:status IS NULL OR e.status = :status) AND (:issuePurpose IS NULL OR e.issuePurpose = :issuePurpose) AND (:referenceType IS NULL OR e.referenceType = :referenceType OR (:referenceType = 'MULTI_TECH' AND e.referenceType IN ('REPAIR', 'ASSEMBLY_ORDER'))) AND (:referenceId IS NULL OR e.referenceId = :referenceId) AND (:partnerId IS NULL OR e.partnerId = :partnerId) AND (:salespersonId IS NULL OR e.salespersonId = :salespersonId) AND (:fromDate IS NULL OR e.docDate >= :fromDate) AND (:toDate IS NULL OR e.docDate <= :toDate) ORDER BY e.updatedAt DESC, e.id DESC")
     ...
     * (VD: Kho A đang DRAFT không chặn tạo phiếu cho Kho B).
     */
    boolean existsByPurchaseOrderIdAndWarehouseIdAndIssuePurposeAndStatusIn(Long purchaseOrderId, Long warehouseId,
            String issuePurpose, List<String> statuses);
