 ...
 * luồng với request nên vẫn thấy SecurityContext của người đang đăng nhập; ở đây chỉ đếm và ghi lại các lần gọi.
 */
public final class AiAgentRun {
    ...
    private AiAgentRun(int maxToolCalls) {
        this.maxToolCalls = maxToolCalls;
    }

    public static AiAgentRun start(int maxToolCalls) {
        AiAgentRun run = new AiAgentRun(maxToolCalls);
        CURRENT.set(run);
        return run;
    }
