...
public class AssemblyOrderService {
    ...
    private static final Set<String> VALID_BOM_STATUSES = Set.of("DRAFT", "PENDING_APPROVAL", "APPROVED", "REJECTED");
    private static final Set<String> VALID_STATUSES = Set.of("DRAFT", "PENDING_APPROVAL", "APPROVED", "IN_PROGRESS", "COMPLETED", "REJECTED", "CANCELLED");
    ...
    public AssemblyBomResponse updateBom(Long id, AssemblyBomRequest request) {
        ...
        AssemblyBom bom = assemblyBomRepository.findByIdWithLines(id)
                .orElseThrow(() -> new BusinessException("Không tìm thấy định mức vật tư"));
        if (!Set.of("DRAFT", "REJECTED").contains(bom.getStatus())) {
            throw new BusinessException(SystemMessage.ASM_ERR_040.getMessage());
        }
        if (assemblyOrderRepository.existsByBomIdAndStatusIn(id,
                List.of("DRAFT", "PENDING_APPROVAL", "APPROVED", "IN_PROGRESS"))) {
    ...
    public AssemblyOrderResponse updateOrderStatus(Long id, String newStatus) {
        ...
        if ("SUBMITTED".equals(status)) {
            ...
            if (ASSEMBLY.equals(order.getOrderType())) {
                ...
                if (order.getTargetVariant().isSerialTracked()) {
    ...
    private void rebuildBomLines(AssemblyBom bom, List<AssemblyBomLineRequest> lines) {
        ...
        for (AssemblyBomLineRequest requestLine : lines) {
            ...
            AssemblyBomLine line = AssemblyBomLine.builder()
                    ...
                    .warrantyMonths(requestLine.getWarrantyMonths() != null ? requestLine.getWarrantyMonths() : component.getWarrantyMonths())
                    .build();
    ...
    private AssemblyBomResponse toBomResponse(AssemblyBom bom) {
        ...
        return AssemblyBomResponse.builder()
                ...
                .submittedBy(bom.getSubmittedBy())
                .submittedAt(bom.getSubmittedAt())
                .approvedBy(bom.getApprovedBy())
                .approvedAt(bom.getApprovedAt())
                .rejectedBy(bom.getRejectedBy())
                .rejectedAt(bom.getRejectedAt())
                .rejectionReason(bom.getRejectionReason())
    ...
    private AssemblyBomLineResponse toBomLineResponse(AssemblyBomLine line) {
        ...
        return AssemblyBomLineResponse.builder()
                ...
                .warrantyMonths(line.getWarrantyMonths() != null ? line.getWarrantyMonths() : (variant != null ? variant.getWarrantyMonths() : null))
                .build();
    }
    ...
    private AssemblyOrderResponse toOrderResponse(AssemblyOrder order) {
        ...
        return AssemblyOrderResponse.builder()
                ...
                .cancelRequestedBy(order.getCancelRequestedBy())
                .cancelRequestedAt(order.getCancelRequestedAt())
                .cancellationReason(order.getCancellationReason())
                .cancelConfirmedBy(order.getCancelConfirmedBy())
                .cancelConfirmedAt(order.getCancelConfirmedAt())
                .cancelledBy(order.getCancelledBy())
                .cancelledAt(order.getCancelledAt())
                .cancellationSettlementStatus(order.getCancellationSettlementStatus())
    ...
    private AssemblyOrderLineResponse toOrderLineResponse(AssemblyOrderLine line) {
        ...
        return AssemblyOrderLineResponse.builder()
                ...
                .note(line.getNote())
                .trackSerial(variant != null && variant.isSerialTracked())
                .build();
    }
