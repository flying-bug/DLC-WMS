...
function CreateImportSlipPage() {
  ...
  const handleOcrPreviewQuickAdd = (index, rawProductName, unit, category, warrantyMonths) => {
    ...
    setOcrQuickAddUnitName(unit || 'Cái');
    setOcrQuickAddCategoryName(category || '');
    setOcrQuickAddWarrantyMonths(warrantyMonths || 0);
  ...
  const handleOcrFile = async (file) => {
    try {
      setOcrLoading(true);
      const res = await scanImportSlipOcr(file, form.partnerId || null);
      const data = res.data?.data || res.data;
      handleOcrSuccess(data);
      setShowOcrModal(false);
    } catch (err) {
      console.error('OCR error:', err);
      showToast('error', err.response?.data?.userMessage || 'Không thể nhận diện hình ảnh/PDF hóa đơn');
    } finally {
      setOcrLoading(false);
    }
  };
  ...
  const confirmOcrPreview = (previewData) => {
    ...
    if (previewData.matchedSupplierId && !form.partnerId) {
      setForm(prev => ({
        ...prev,
        partnerId: previewData.matchedSupplierId,
        partnerName: previewData.matchedSupplierName || ''
      }));
    }
    ...
    if (previewData.items && previewData.items.length > 0) {
      ...
      setItems(prev => {
        const hasDummyOnly = prev.length === 1 && !prev[0].variantId;
        return hasDummyOnly ? newItems : [...prev, ...newItems];
      });
    }
  ...
  const [items, setItems] = useState(() => {
    ...
    if (stocktakeData && stocktakeData.lines && stocktakeData.lines.length > 0) {
      ...
      return stocktakeLines.length > 0 ? stocktakeLines.map(line => ({
        ...emptyLine(stocktakeData.warehouseId ? String(stocktakeData.warehouseId) : ''),
        variantId: String(line.variantId),
        warehouseId: stocktakeData.warehouseId ? String(stocktakeData.warehouseId) : '',
  ...
  const handlePrint = (printMode = 'SUMMARY') => {
    const slipToPrint = savedSlip || form || {};
    printImportSlip(slipToPrint, {
      supplier: suppliers.find(s => String(s.id) === String(slipToPrint.partnerId || form.partnerId)),
      customer: customers.find(c => String(c.id) === String(slipToPrint.partnerId || form.partnerId)),
                ...
                {form.referenceId ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '8px' }}>
                    ...
                    <i
                      ...
                      onClick={() => setForm(prev => ({ ...prev, referenceType: '', referenceId: '', referenceCode: '', purchaseOrderId: '' }))}
                  ...
                  <input
                    ...
                    className="misa-input"
                    style={{ marginTop: '8px' }}
                    value={form.attachedDoc || ''}
                    onChange={(e) => handleFormChange('attachedDoc', e.target.value)}
            ...
            <div className={styles.cardBody}>
              <div className="misa-form-group" style={{ marginBottom: '16px' }}>
                ...
                <input
                  ...
                  className="misa-input"
                  value={form.docCode}
                  onChange={(e) => handleFormChange('docCode', e.target.value)}
                ...
                {items.map((item, index) => {
                  ...
                  return (
                    <tr key={item.localId}>
                      ...
                      {showPricing && (
                        <td align="right">
                          <input type="number" min="0" max="10" step="any" className="misa-input" style={{ height: '32px', padding: '0 8px', width: '55px', textAlign: 'right', fontSize: '13px' }} value={item.vatPercent !== undefined ? item.vatPercent : ''} onChange={(e) => handleItemChange(item.localId, 'vatPercent', e.target.value)} />
                        </td>
                      )}
                      <td>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', minWidth: '350px' }}>
              ...
              {showPricing ? (
                ...
                <table style={{ width: '100%', fontSize: '13px' }}>
                  <tbody>
                    <tr>
                      <td style={{ padding: '6px 0', fontWeight: 'bold' }}>Tổng số lượng nhập</td>
