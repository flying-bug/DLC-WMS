...
const money = (value) => `${Number(value || 0).toLocaleString('vi-VN')} Ã„â€˜`;
...
function UpdateExportSlipPage() {
      ...
      try {
        ...
        if (detail) {
          ...
          setForm({
            ...
            referenceCode: detail.referenceCode || (detail.referenceId ? `Tham chiÃ¡ÂºÂ¿u #${detail.referenceId}` : ''),
          });
        ...
        setError(err.response?.data?.userMessage || 'KhÃƒÂ´ng tÃ¡ÂºÂ£i Ã„â€˜Ã†Â°Ã¡Â»Â£c phiÃ¡ÂºÂ¿u xuÃ¡ÂºÂ¥t kho');
      } finally {
        setLoading(false);
      }
    };
  ...
  const submit = async (status, shouldPost = false) => {
    ...
    try {
      ...
      navigate('/export-slips', { state: { toastMessage: shouldPost ? 'Ghi sÃ¡Â»â€¢ phiÃ¡ÂºÂ¿u xuÃ¡ÂºÂ¥t kho thÃƒÂ nh cÃƒÂ´ng!' : 'CÃ¡ÂºÂ­p nhÃ¡ÂºÂ­t phiÃ¡ÂºÂ¿u xuÃ¡ÂºÂ¥t kho thÃƒÂ nh cÃƒÂ´ng!', toastType: 'success' } });
    } catch (err) {
      showToast('error', err.response?.data?.userMessage || err.response?.data?.devMessage || 'KhÃƒÂ´ng cÃ¡ÂºÂ­p nhÃ¡ÂºÂ­t Ã„â€˜Ã†Â°Ã¡Â»Â£c phiÃ¡ÂºÂ¿u xuÃ¡ÂºÂ¥t kho');
    } finally {
      setSaving(false);
    }
  };
    ...
    <AdminLayout>
      <div className={styles.pageHeader}>
        <a href="#" className={styles.backLink} onClick={(e) => { e.preventDefault(); navigate('/export-slips'); }}>
          <i className="bi bi-arrow-left"></i> CÃ¡ÂºÂ­p nhÃ¡ÂºÂ­t phiÃ¡ÂºÂ¿u xuÃ¡ÂºÂ¥t kho {form.docCode ? form.docCode : ''}
        </a>
      </div>
      ...
      <div className={styles.pageBody}>
        ...
        {loading ? (
          <div className={styles.card}>
            <div className={styles.cardBody}>Ã„Âang tÃ¡ÂºÂ£i dÃ¡Â»Â¯ liÃ¡Â»â€¡u...</div>
          </div>
        ) : (
          <>
